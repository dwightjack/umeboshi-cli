...
module.exports = (files, metalsmith, done) => {
    ...
    async.each(keys, (file, next) => {
        ...
        if (!/{{([^{}]+)}}/g.test(str)) {
            logger.verbose(`[render] File "${file}" copied`);
            next();
            return;
        }
        ...
        render(str, metadata, (err, res) => {
            ...
            files[file].contents = Buffer.from(res); //eslint-disable-line no-param-reassign
            logger.verbose(`[render] File "${file}" parsed and copied`);
            next();
        });
    }, done);
